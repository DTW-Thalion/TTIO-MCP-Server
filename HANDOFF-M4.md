# HANDOFF-M4.md — MPEG-O-MCP M4: Cloud I/O

## Context

M2 shipped the catalog with local-only URIs. M3 added query tools
(including the one tool — `mpgo_get_spectrum` — that reopens the
...
The win came for free from MPEG-O v1.0.0: `SpectralDataset.open`
already accepts fsspec URIs (`s3://`, `gs://`, `az://`, `http(s)://`,
`gcs://`, `abfs(s)://`). MPEG-O streams HDF5 metadata and reads
...
- **fsspec kwargs threading.** `Config` reads
  `MPGO_MCP_FSSPEC_KWARGS` (JSON object) and uses it as the default
  for every cloud call. `mpgo_register_file` and `mpgo_get_spectrum`
...
- **No schema changes.** `files.uri` already holds arbitrary strings;
  ...
  through M4.
- **Documentation on reverify.** `mpgo_reverify` for cloud files
...
- **Encryption + signed bundles.** M5. The MPEG-O
...
- **Caching.** fsspec has a `simplecache://` wrapper; we don't enable
  it in M4 — each `mpgo_get_spectrum` call re-opens. If the clients
...
MPGO_MCP_FSSPEC_KWARGS   JSON object, e.g.
...
### `mpgo_register_file` — add `fsspec_kwargs`
...
### `mpgo_get_spectrum` — add `fsspec_kwargs`
...
- **No new codes** in M4. A cloud fetch that 404s surfaces as
  `resolve_failed`; MPEG-O refusing to parse surfaces as `not_mpgo`;
...
- [ ] `mpgo_get_spectrum` reads a single spectrum from the S3-hosted
      file and returns the same channels the local fixture does.
- [ ] Passing `fsspec_kwargs` per-call overrides `MPGO_MCP_FSSPEC_KWARGS`.
