# HANDOFF-M6.md — MPEG-O-MCP M6: Cloud Push + Encrypt-on-Upload
...
M6 fills the write side. The new `mpgo_push_file` tool uploads a local
...
- **`mpgo_push_file` tool.** Streams a local `.mpgo` to a writable
...
- **Post-upload catalog flag.** After a successful encrypted push, the
  ...
  encrypted — MPEG-O metadata (title, runs, identifications,
...
- **Documented manual workflow.** For a file that *already* lives in
  ...
  (1) pull it down,
  (2) `mpgo_encrypt_file` on the local copy,
  (3) `mpgo_push_file` with no `key_id` (ciphertext already).
...
- **Streaming encryption.** The encrypt-on-push path fully stages the
  file on local disk before uploading. MPEG-O's
  `encrypt_with_key` operates on an opened HDF5 file, not a stream —
  a true streaming path would require a new MPEG-O API. Deferred.
...
src/mpeg_o_mcp/
└── tools/
    ├── __init__.py      # UPDATED — registers mpgo_push_file (11 tools total)
    └── push_file.py     # NEW — mpgo_push_file handler
